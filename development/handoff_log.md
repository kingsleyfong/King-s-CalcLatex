...
User resubmitted v3.8.3 and got back a much shorter review report: only 2 blocking Errors this time (everything else was non-blocking noise, or dependency checks passing clean). (1) The settings page's new heading text ("Kings CalcLaTeX Settings," itself added by Part 49's heading-conversion work) tripped two rules specific to `Setting().setHeading()`-based headings that don't apply to raw HTML headings: can't say "settings," can't repeat the plugin name. Fixed by deleting the heading outright -- Obsidian's own settings-tab UI already shows the plugin name as context, so it was pure redundancy anyway. (2) `latex-modal.ts`'s 27 flagged `!important` style writes -- the file explicitly held off in Part 49 ("hold off on this one for now") -- was now the ONLY remaining blocker. Asked the user again given it's now the sole blocker; they said fix it, carefully.

**The discovery that made this tractable**: rather than guessing at a uniform CSS-var-based refactor for all 27 lines, cross-referenced the exact flagged line numbers against the file's real content and found a pattern -- lines with a STATIC string-literal value (`"fixed"`, `"auto"`, `"50%"`) were flagged; lines with a VARIABLE value (`bottom`, `left`, `maxWidth` -- i.e. the actual rect-derived pixel math from the carefully-tuned v3.8.1 fix) were NOT flagged, even when sitting one line apart in the same `setProperty()` block. Verified this against 5 separate spot-checks (315/316, 405-407, 416/418, 433/435, plus `dot.style.backgroundColor = color.hex` elsewhere in the file being unflagged) before trusting it. This meant `obsidianmd/no-static-styles-assignment` only cares about literal values -- so the actual dynamic positioning math needed ZERO changes, only the static "shape" properties (position:fixed, the opposite-axis auto value, transform, center's fixed 50%s) needed to move into CSS classes.

Called `advisor` twice for this file specifically, given it's the exact code controlling the v3.8.1 width-cap/cursor-pan behavior and had already been deferred twice:

**First call** (before any edits): confirmed the static-vs-dynamic split was the right read and validated the general CSS-class approach, then caught three things worth fixing before writing code: (a) the MutationObserver-loop-prevention guard (`applyIfChanged`) needed re-keying to compare whatever JS actually still writes, not the properties being moved out to CSS -- getting this wrong would silently break the infinite-loop guard; (b) roughly a third of the 27 lines were already redundant with existing CSS rules (`.modal-container:has(.excalidraw-LatexPrompt)` and its `.modal` descendant rule) that had been sitting in `styles.css` unused/masked this whole time -- the deleted inline `!important` writes had simply been winning over them silently; (c) a real cascade-tier risk worth checking empirically rather than assuming: inline `!important` beats stylesheet `!important`, so moving off inline for the tooltip-positioning code specifically needed verification against Excalidraw's own bundled CSS, not just trust.

Checked Excalidraw's actual installed `styles.css` (`.obsidian/plugins/obsidian-excalidraw-plugin/styles.css`) directly rather than guessing: confirmed it uses CSS Anchor Positioning with `!important` on `.cm-tooltip-cursor.cm-tooltip` (`position:fixed!important; top:unset!important`) -- and confirmed via this project's own `math_tooltip.ts` that our math-preview tooltip does carry that exact class, so the conflict risk was real, not hypothetical. Also checked whether Excalidraw's `.modal.excalidraw-modal { transform:none!important }` rule could clash with the modal-position code -- traced through Excalidraw's `main.js` and confirmed the LaTeX prompt modal is a plain vanilla Obsidian `Modal` (`this.contentEl.addClass("excalidraw-LatexPrompt")`, no `excalidraw-modal` class anywhere), so that particular rule never applies here.

**Second call** (after implementing, before shipping): confirmed the static/dynamic split held up under review, then caught two more things: (a) lines 1021/1027's `:has()`-based CSS rules had literally never been load-bearing before this change -- the inline `!important` writes being deleted had been masking them the entire time, so depending on them for the first time now carried real, typecheck-invisible risk (if `:has()` didn't resolve as expected in some Obsidian/Electron Chromium version, the symptom would be the modal backdrop swallowing clicks instead of passing them through to the canvas); (b) a minor ordering bug in `positionTooltip()` where a newly-added `classList.add()` call sat after an early-return pixel-comparison guard, meaning a freshly-positioned tooltip that happened to already be at the right pixel coordinates would never get the class.

**Fixes applied**:
- `positionTooltip()`: moved `position:fixed`/`transform:none` (static) to `.kcl-latex-tooltip-positioned`, applied unconditionally above the pixel-guard (not gated by it, per the second advisor catch); `left`/`top` (dynamic) untouched, confirmed necessary as direct inline `!important` writes to reliably beat Excalidraw's own `!important` tooltip-anchor-positioning CSS.
- `applyModalPosition()`: replaced the reliance on the previously-dormant `:has()` CSS rules with explicit `classList.add()` calls (`.kcl-latex-modal-container` on the modal-container, `.kcl-latex-modal-positioned` on the modal itself -- folding in the pointer-events/margin/box-shadow that used to live in the now-removed `.modal-container:has(...) .modal` rule), per the second advisor call. Added 3 position-variant classes (`.kcl-latex-modal-pos-bottom/top/center`) for the static shape only; kept the dynamic `bottom`/`left`/`top`/`max-width` pixel writes completely untouched as direct inline `!important`. Re-keyed `applyIfChanged`'s loop guard to check the surviving dynamic inline props plus whether the correct variant class is active. Added explicit `style.removeProperty()` calls on every variant switch (bottom→top, top→bottom, either→center) -- without this, a stale inline `!important` value left over from a *previous* position-mode application would silently out-rank the new variant's class-based `!important` rule (inline always wins over stylesheet regardless of recency), which would have been the single most likely way this refactor could have introduced a real regression. This was the one part of the fix requiring genuine care beyond mechanical translation.
- `settings.ts`: deleted the offending top-level heading; removed its now-orphaned `.kcl-settings-title` CSS class.
- Three smaller static writes (`suggestion` hide, `dotsContainer` flex/gap, `settingsGrid` show/hide) converted to `setCssStyles()` / new CSS classes respectively -- straightforward, no design decisions needed.

Verified no regressions via the same `git stash`/`git stash pop` before/after `npx tsc --noEmit` comparison technique used in Part 49; zero new errors introduced. `npm run build` clean, synced to vault. Considered installing `eslint-plugin-obsidianmd` locally to pre-verify against the exact rule set before resubmitting (advisor suggested this to avoid a third review cycle) -- checked its peer dependencies and found it pins `obsidian@1.8.7` against this project's `obsidian: latest`, and even with it installed there's no guarantee the locally-runnable config exactly matches whatever flat-config the server-side reviewer actually uses -- decided the setup cost and risk of a misleading "all clear" outweighed the benefit, and did a thorough manual diff review instead (the same line-number-based static/dynamic verification technique that got this fix right in the first place).

Shipped as v3.8.4, same conventions as Part 49 (patch bump, `Kingsley Fong <ktcfong@uwaterloo.ca>` authorship, no Claude co-author trailer, tag without `v` prefix, both root and repo-v2 CHANGELOG.md kept in sync).

### Needs live confirmation from user
Resubmit through community.obsidian.md and confirm both Errors are gone. Also, since nothing in this fix could be live-tested in this environment, re-verify in a real split-screen Obsidian session: all four `latexModalPosition` settings (bottom/top/center/cursor) position correctly, the v3.8.1 width-cap + End-key horizontal-pan behavior is unchanged, the math-preview tooltip still places correctly while typing, the box-panel (border/background/thickness/style/padding settings) still expands and collapses correctly, clicking through the modal's backdrop still reaches the canvas underneath (this is the specific behavior that depends on the newly-load-bearing `.kcl-latex-modal-container` class actually applying `pointer-events` correctly), and there's no CPU spin while typing in the modal (which would indicate the MutationObserver loop-guard broke).
...
### Two bugs found in the user's v3.5.1 test log, one fixed, one deferred
1. **Modal live preview: `ReferenceError: MathJax is not defined`, every single render attempt.** Root cause: Obsidian only lazy-loads the global MathJax runtime the first time it's actually needed, normally triggered by rendering markdown math in a note. Excalidraw's "Edit LaTeX" modal is a floating overlay that never goes through normal markdown rendering, so by the time this modal's `renderMath()` call fired, MathJax had never been loaded in this session at all. **Fixed**: await Obsidian's own public `loadMathJax()` loader before the first `renderMath()` call (`latex-modal.ts`'s `injectLivePreview`), gated behind a `mathJaxReady` flag so early calls before the load resolves are cheap no-ops instead of throwing.
2. **Tabstop navigation lands at a stale position after typing inside an earlier tabstop.** Traced precisely: after `10rd` → `10^{}` expansion (2 tabstops: index0 inside the braces, index1 right after), typing `1` then `0` inside the braces (`10^{10}`) shifts everything after the insertion point by 2 characters -- but `TabstopManager.tabstops[1]`'s stored `from`/`to` was never updated to account for that shift. Confirmed: `TabstopManager.adjustForEdit()` exists (`tabstop-manager.ts`) and does the right thing, but grepped `snippet-engine.ts` end-to-end and confirmed it is **never called anywhere** in the input-handling flow -- dead code. First Tab press after typing inside the tabstop jumped the cursor to the stale pre-edit position (33) instead of the correct post-edit one (35), moving it *backward*. **Not fixed this session** -- became moot for the modal because of the pivot below, but still affects the canvas `<textarea>` path, which still runs the hand-rolled engine (see "Known follow-up" below).

### The pivot: user asked directly why the modal doesn't just use the real, already-working, fully-vendored LaTeX Suite instead of a hand-built approximation
Every modal bug fought since Part 40 (CM6-microtask staleness, a reentrancy-guard regression caught in advisor review, the auto-fraction `$`-swallowing bug, input/keydown listener-phase races, and now the tabstop-staleness bug above) is a bug *in the hand-rolled `SnippetEngine`/`TextSurface` reimplementation* -- none of them exist in the real vendored engine (`src/latex-suite/**`), because that's the same, already-correct code running successfully in the main note editor. The user's instinct to stop reimplementing and just use the real thing was right. One correction made to the *how*: not Excalidraw's own "detect the community 'Latex Suite' plugin" integration path (spoofing `app.plugins.plugins["latex-suite"]` previously broke Excalidraw's right-click "Edit LaTeX", double-click editing, and the `Ctrl+\` shortcut entirely in Parts 4-5, because Excalidraw calls methods on that object expecting the real plugin's exact API shape) -- instead, inject our own engine's real CM6 `Extension[]` directly into the modal's own already-working EditorView integration, via CM6's own `StateEffect.appendConfig` primitive. Same outcome the user wants, without touching Excalidraw's plugin registry at all.

**Feasibility research (code reading, no live test needed for this part):**
- Grepped all of `src/latex-suite/{snippets,editor_extensions,features,utils}/**` for `app.workspace`/`MarkdownView`/`activeEditor`/`.app` -- zero hits. The entire engine (`provider.ts`'s `initLaTeXSuiteEngine`, `latex_suite.ts`'s `runSnippets`/`onInput`/`keyboardEventPlugin`/`getKeymaps`, tabstops, matrix shortcuts, tabout, auto-fraction) is pure CM6, driven only by `EditorView`/`EditorState`, with zero dependency on being embedded in an actual Obsidian `MarkdownView`/`Editor`. Confirms it's injectable into any CM6 `EditorView`, foreign or not.
- The one real blocker: math-bounds detection (`utils/context.ts`'s `mathBoundsPlugin`/`contextPlugin`) is coupled to Obsidian's markdown syntax tree (specific node names like `formatting_formatting-math_...`) to figure out "is the cursor inside math right now" -- which underpins not just conceal/bracket-coloring/the math-preview tooltip, but the *core* snippet engine's own mode-gating too. Excalidraw's modal EditorView has no markdown language loaded, so this would silently report "never in math" everywhere, same failure shape as every earlier bug in this project caused by a silently-degrading dependency.
- Traced every other `syntaxTree`-dependent file (`math_tooltip.ts`, `tokenizer.ts` references) and confirmed they degrade gracefully with no language loaded (return sensible defaults, don't throw) -- `utils/context.ts`'s math-bounds plugin was the only one that needed an actual fix, not just a "will this crash" check.

**Feasibility research (live, could NOT be determined by reading code):** does `StateEffect.appendConfig` actually survive on Excalidraw's EditorView, given it's a foreign, React-managed instance whose lifecycle this plugin doesn't control? If React tears down and rebuilds the view mid-session, anything appended is silently discarded along with it -- and so would the real engine's snippet-queue/tabstop CM6 StateFields be. **Verified live before committing to the surgery**: shipped a throwaway build with a single, dependency-free `EditorView.updateListener.of(...)` probe dispatched via `appendConfig`, and asked the user to test. Their resulting log showed it firing continuously through a full typing session -- including immediately after a Tab press -- confirming the view is stable, not rebuilt per-render. Green light to proceed, per an explicit advisor consultation on this exact question beforehand (advisor had initially advised against full-engine reuse in Part 40 due to the syntax-tree coupling; retracted that once shown the coupling was a single patchable point in code this project owns, and gave the appendConfig-survival question as the one remaining blocker to check first).

### What was actually built
1. **`src/latex-suite/utils/context.ts`: added `alwaysMathFacet`** (exported `Facet<boolean, boolean>`, `combine: values => values.some(Boolean)`), explicitly commented as a KCL fork addition (not upstream `artisticat1/obsidian-latex-suite`) so a future upstream-fidelity diff/audit (per this project's CLAUDE.md rule 0(e)) doesn't mistake it for drift and revert it. When set on an `EditorState`, `MathBoundsPlugin.inMathBound()` and `.getEquations()` (the only two entry points anything outside this file uses to query math bounds -- confirmed via grep of every external callsite: `conceal_fns.ts`, `highlight_brackets.ts`, `context.ts` itself) short-circuit to treating the ENTIRE document as one math region, before ever touching `syntaxTree()`. Deliberately checked at the very top of `inMathBound`, not by pre-populating `this.mathBounds` and letting the existing binary-search logic run: that logic treats being exactly AT a bound's `outer_start`/`outer_end` as "outside" (correct for a real `$`-delimited equation, ambiguous right at the delimiter) -- for a "whole buffer is math" declaration, that edge case is the single most common cursor position while actively typing (cursor sits at `doc.length` after every appended keystroke), so relying on the existing logic would have silently failed exactly when it mattered most.
2. **`src/excalidraw/latex-modal.ts`: new `injectRealOrFallbackSnippetEngine()`** replaces the old always-hand-rolled `injectSnippetEngine()` call site. Tries `getLaTeXSuiteEngineExtension(this.plugin)` (the exact same cached `Extension[]` the main note editor uses -- same snippets, same user settings, same conceal/bracket-color/tooltip toggles) plus `alwaysMathFacet.of(true)`, dispatched via `editorView.dispatch({ effects: StateEffect.appendConfig.of([...]) })`. Falls back to the renamed `injectHandRolledSnippetEngineFallback()` (the old hand-rolled `SnippetEngine`/`CM6Surface` path, unchanged) only if the real-engine dispatch throws -- so a regression in the real-engine path degrades to previously-working behavior instead of to nothing. The hand-rolled engine remains load-bearing and unremoved for the canvas `<textarea>` path, which has no CM6 to inject into at all.
3. **`LaTexModalEnhancer` constructor gained an optional `plugin: KingsCalcLatexPlugin` parameter** (`companion-manager.ts` updated to pass `this.plugin`), needed to call `getLaTeXSuiteEngineExtension(plugin)`, which reads `plugin.settings` to build the real engine's config exactly as the main editor does.
4. **Fixed the MathJax loading bug** (see above) in the same pass, since it was found in the same log and is independent of the pivot.
5. **Removed the temporary `appendConfig` smoke-test probe** (its job -- confirming survivability -- is done) and its call site, per the plan set when it was added.

### ⚠️ Needs live confirmation before this is trusted at all -- this is a pivot, not a polish pass
1. Does the modal even still open/function at all? (Sanity check first -- `editorView.dispatch` with a large appended extension array, including a full `keymap.of(...)`, could in principle conflict with something Excalidraw's own editor depends on. No live evidence either way yet.)
2. Type `mk`/`sr`/Greek letters/auto-fraction (`/`) in the modal -- do they expand correctly, including the previously-broken `$`-adjacent case? (The real engine's auto-fraction operates on real math bounds that never contain a literal `$` in the first place, for this buffer -- expect this class of bug to not exist at all, not just be patched.)
3. Tab navigation between tabstops -- does it land at the CORRECT position now (real engine's tabstop StateField, not the hand-rolled `TabstopManager` with its dead `adjustForEdit`)?
4. Conceal, bracket-color-matching, math-preview-tooltip -- do any of these visibly render at all now? (These were never wired to anything before this session; first real test.)
5. Does the live preview (separate, `injectLivePreview`, MathJax-loading fix) now actually render an equation instead of throwing?
6. If anything in 1-4 is broken or absent, check console for `[KCL-DEBUG] Modal: real LaTeX Suite engine injection failed, falling back to hand-rolled engine` -- if that fired, the real-engine path threw and silently fell back; the thrown error will be logged right there and is the actual next thing to fix, not a fresh investigation.

### Known follow-up, deferred (not blocking this pivot)
Canvas tabstop staleness (`TabstopManager.adjustForEdit()` dead code, found this session) still affects the canvas `<textarea>` path, unaffected by this pivot since there's no CM6 there to inject into. Deferred because the user's most recent complaints were modal-specific and canvas conversion/snippets were explicitly confirmed as working well.
...
### Context
Part 39 fixed the Excalidraw companion's dead `onload()`. This session picked up from the user confirming `mk`/`sr`/auto-fraction expansion now genuinely works on the canvas, but reporting two remaining problems: (1) finishing an equation in a canvas text box and clicking away never converts it to a rendered LaTeX SVG, and (2) the "Edit LaTeX" modal popup has no snippet/shortcut expansion at all.

### Finding 1: canvas blur-to-equation doesn't exist anywhere — it had to be built new
Investigated the actual installed `obsidian-excalidraw-plugin/main.js` bundle (via Node.js string search — no TS types available) and confirmed: Excalidraw has no built-in feature that converts a plain text element into a rendered equation image on blur. This was never a regression to hunt down; it needed to be built. Confirmed this explicitly with the user before proceeding ("Do you want me to build canvas-blur-to-equation-image as a new feature? — Yes, build it").

**Built**: `TextareaInterceptor.handleAttach()`'s blur handler (`interceptor.ts`) now captures `textarea.value` synchronously and fires a new `onBlurCommit(text, view)` callback before the textarea is torn down. `ExcalidrawCompanionManager.onTextareaBlurCommit()` (new method, `companion-manager.ts`) checks whether the committed text is a full `$...$`/`$$...$$` math block (`extractFullMathBlock`), and if so drives Excalidraw's own public automation API (`window.ExcalidrawAutomate`) to replace it with a real rendered equation: find the matching text element in the scene, delete it, then `ea.addLaTex(x, y, latex, ...)` + `ea.addElementsToView(...)`.

Two bugs found and fixed while building this, both confirmed by the user's own pasted stack traces/logs:
- **`getAPI` `this`-binding crash**: `const getAPI = win.ExcalidrawAutomate?.getAPI; getAPI(view)` throws `TypeError: Cannot read properties of undefined (reading 'plugin')` because destructuring a method off an object strips its `this` binding. `getAPI` reads internal state via `this` under the hood. Fixed by calling it as `win.ExcalidrawAutomate.getAPI(view)`, never destructured.
- **Leftover original text box after successful equation render**: the delete-the-original-text-element mutation ran *after* `await ea.addElementsToView(...)`, and that call's scene commit happens asynchronously through Excalidraw's own React update cycle — a `getSceneElements()` read immediately after can race that commit and silently drop the deletion (image renders correctly, but the raw-text element it replaced never disappears — both visible at once). **Fixed by reordering**: delete the plain-text element via a synchronous `api.updateScene()` call *first*, then call `addLaTex`/`addElementsToView`. This reorder is the one part of Finding 1 not yet re-confirmed by the user (their last report predates this specific change landing).

### Finding 2: modal had zero snippet engine wired, and two real timing bugs surfaced fixing it
`latex-modal.ts`'s `LaTexModalEnhancer` only ever suppressed Excalidraw's own "Install the Latex Suite plugin" nag — no substitute engine was ever attached. Built `text-surface.ts`: a `TextSurface` interface (`getValue`/`getSelectionStart`/`getSelectionEnd`/`setValue`) with two implementations — `TextareaSurface` (thin wrapper around the existing `updateTextarea` helper, used for the canvas) and `CM6Surface` (wraps a CodeMirror 6 `EditorView` — `view.state.doc.toString()`, `view.state.selection.main`, `view.dispatch({changes, selection})`). Refactored `snippet-engine.ts`'s `SnippetEngine` from hardcoded `HTMLTextAreaElement` access to driving through a `TextSurface`, so the exact same matching/tabstop/auto-fraction/matrix logic now serves both the canvas textarea and the modal's real CM6 editor. `latex-modal.ts` now constructs a `SnippetEngine`, feeds it the same snippet set built in `companion-manager.ts`, and attaches it via `injectSnippetEngine()`.

Three real bugs surfaced getting this correct, in order of discovery:

1. **Capture-phase ancestor listeners for both `input` and `keydown` was wrong for one of them.** `keydown` genuinely needs a capture-phase listener on an ancestor (`.cm-editor`) — CM6 registers its own Tab/Backspace handling directly on `.cm-content` at editor-construction time, and listeners on the *same* element fire in registration order regardless of capture, so only an ancestor capture-phase listener is guaranteed to see the event (and be able to `preventDefault()`) before CM6's own handler runs. But `input` does NOT want this — attaching it the same way means reading state *during the capture descent*, before the event has even reached its target, i.e. before CM6 has processed the keystroke at all. Fixed by splitting `attachSurface()` into independent `inputTarget`/`keydownTarget` parameters — `input` stays directly on `cmContent` (target phase, fires after CM6's own same-element handling), `keydown` goes on the ancestor.

2. **A `keyup` fallback (mirroring a pattern from `injectLivePreview`) caused a runaway brace-insertion loop.** Confirmed via the user's pasted console log showing `M_{}}}}}}}}}}}}}}}}}}}}}}}}}}}}` growing unboundedly on every keystroke. Root cause: firing the match-and-expand logic twice per keystroke (once on `input`, once on `keyup`) double-processes bracket-pair snippets (`_` → `_{$0}$1`) — after the first pass inserts the closing `{`, the cursor sits immediately after it, so the second pass re-matches that same trailing `{` and re-expands again, and the pattern repeats forever. Fixed by removing the `keyup` fallback entirely — `input`-only, no hedge.

3. **CM6 MutationObserver microtask staleness — the loop's actual root cause survived bug #2's fix, with an identical symptom.** Even with `keyup` gone, the modal kept producing the same runaway-brace pattern. Traced via the user's pasted log: typing `M` → `onInput` logged `text=""`; typing `_` next → `onInput` logged `text="M"` — every read was exactly one keystroke behind. Root cause: CM6 reconciles native contenteditable DOM mutations through its own internal `MutationObserver`, which is a **microtask**, and that microtask is queued *after* the synchronous native `input` DOM event has already fired and returned. Reading `editorView.state` synchronously inside an `input` handler therefore always sees the state from before the current keystroke was applied. A plain `<textarea>`'s `.value` has no such indirection (synchronously current), so this was invisible on canvas and modal-only. **Fixed** by splitting `onInput(e)` into a thin synchronous wrapper and a new `processInput()` that does the actual read/match/expand work, with the wrapper calling `queueMicrotask(() => this.processInput())` — deferring the read until after CM6's own microtask has already run.

4. **Caught in advisor review, before the user ever saw it: the microtask defer silently broke the `isExpanding` re-entrancy guard on the canvas path.** The original (pre-this-session) `onInput` checked `isExpanding` synchronously as its first line — this is what prevents `updateTextarea`'s (`interceptor.ts`) synchronous re-dispatch of a second native `input` event (part of how it bypasses React's controlled-input reversion for the canvas textarea) from re-entering the matcher mid-expansion. Moving that check inside the now-deferred `processInput()` meant the *re-entrant* call's own deferred check would run only after the outer call had already reset `isExpanding` back to `false` — so it would proceed instead of bailing, re-matching the just-inserted `{` of a bracket-pair snippet and reintroducing the exact same runaway-brace cascade on the **canvas**, as a regression, while "fixing" the modal. This was never exercised by any test so far in this session because `mk`'s cursor lands between `$$`, which doesn't trigger the bracket-snippet path — only `_`/`(`/`[`/`{` do. Fixed by keeping the `isExpanding` check synchronous, directly in `onInput`, before scheduling the microtask; `processInput()` keeps its own copy of the check too (defense for the CM6 path, which can't hit this specific race since `CM6Surface.setValue` uses `editorView.dispatch`, not a native event dispatch).

### Build status
`npx tsc --noEmit` clean for all `excalidraw/` files. `npm run build` succeeded, synced to `.obsidian/plugins/kings-calclatex/`.

### ⚠️ Needs live confirmation in Obsidian — this is the priority list for next session or next message
1. **Canvas regression check (the one the advisor caught):** type `_`, `(`, `[`, or `{` on an Excalidraw canvas text box — NOT just `mk` — and confirm no runaway brace/bracket cascade. This is the discriminating test; `mk` alone will not reveal a regression here.
2. **Canvas blur-to-equation, specifically the delete-before-insert reorder:** finish a `$...$` equation, click away, confirm the SVG equation appears AND the original plain-text element is actually gone (not just hidden behind/near the image).
3. **Modal snippet expansion, with the console log pasted:** type `sr`, `mk`, or a subscript `_` inside the "Edit LaTeX" modal, and paste the `[KCL-DEBUG] onInput: text=...` log lines. If `text=` is still one keystroke behind what was actually typed, the microtask-defer theory is wrong for this CM6 build (possibly flushes via `requestAnimationFrame` instead) — do not try more defer-timing guesses; switch to CM6's native `updateListener` extension instead, which fires with guaranteed-fresh state (with its own caveat: it forbids synchronous `dispatch()` from inside it, so the expansion write-back would need its own defer/queue).

---

## Session: 2026-07-23 (Part 39) — Excalidraw companion was 100% dead: `onload()` threw on its first line

### Status: 🟢 Root cause fixed & verified | 🟡 Two more fixes applied but NOT yet verified live (see below) | Needs Obsidian confirmation

### The one bug that explains everything
User reported three symptoms: text→LaTeX-SVG conversion completely broken, the modification modal's position wrong, and the modal still showing "Install the 'Latex Suite' plugin...". Investigation found **`ExcalidrawCompanionManager.onload()` threw on its very first async call**, before creating the snippet engine, the blur interceptor, the preview tooltip, the modal enhancer, or registering any leaf-watching — meaning **none of the Excalidraw companion has been running at all**, for however long this bug has existed. This single fact explains all three symptoms at once: no interceptor running → no custom blur handling → Excalidraw's own native (unrelated) equation editor and its own default positioning take over untouched → its own missing-plugin warning shows because nothing suppresses it.

**Root cause**: `companion-manager.ts` called `parseSnippetVariables(DEFAULT_SNIPPET_VARIABLES, ...)` and `parseSnippets(DEFAULT_SNIPPETS, ...)` from `latex-suite/snippets/parse.ts`. Those two functions expect a raw **JS source string** to `eval` via a Blob-URL dynamic import (upstream's original architecture, before this fork pre-compiled snippet data to satisfy Obsidian's CSP — see Part 36). `DEFAULT_SNIPPET_VARIABLES`/`DEFAULT_SNIPPETS` are already-compiled objects/arrays, not strings — string-concatenating an object produces `"[object Object]"`, which fails to import as valid JS, throwing `"Invalid format"`. **Verified empirically** (bundled the real call in an isolated Node harness, same technique as Part 36): confirmed it throws exactly this error.

### What was fixed (and how confident each fix is)
1. **The `onload()` crash (CONFIRMED FIXED — verified in isolation)**: added `buildExcalidrawSnippets()` in `companion-manager.ts`, which works directly on the pre-compiled `DEFAULT_SNIPPETS`/`DEFAULT_SNIPPET_VARIABLES` — manual `${VAR}` substitution, and compiles regex-flagged snippets (`options` containing `"r"`) into real `RegExp` objects (anchored `(?:pattern)$`, valid flags filtered), matching what `snippet-engine.ts` requires (it does `instanceof RegExp` checks in several places). Verified with a Node harness: 199 snippets converted (200 minus the intentionally-excluded `dm`), Greek/symbol variable substitution confirmed working, 40 regex snippets correctly compiled as real `RegExp` instances. **This is the fix that matters most** — it's what makes the rest of `onload()` (interceptor, tooltip, modal enhancer, leaf watching) run at all for the first time.

2. **Blur-sync restoration in `interceptor.ts` (fix applied, NOT independently verifiable — needs live confirmation)**: the handoff log (Parts 2–3) describes a prior fix where trailing whitespace was trimmed and Excalidraw's internal text state synced *before* its own blur handler ran, making its native `tex2svg`-equivalent conversion fire correctly. That logic was absent from the current `interceptor.ts` (just a bare `setTimeout` detach) — restored it as a capture-phase `blur` listener on the container (fires before Excalidraw's own target-phase listener on the textarea, since `blur` doesn't bubble but does have a capture phase), which trims trailing whitespace and syncs `appState.editingTextElement.text/originalText/rawText` via the Excalidraw API. Confirmed `editingTextElement` and `originalText`/`rawText` are real, current property names by grepping the actual installed `obsidian-excalidraw-plugin` bundle (note: the historical log said `editingElement`, not `editingTextElement` — the property was probably renamed in an Excalidraw update at some point). **Could not verify this actually fixes SVG conversion** — since `onload()` was dead, this code path has never run either; it's entirely possible Excalidraw's native conversion just works correctly once `onload()` completes, independent of this fix. Re-test needed.

3. **Modal position — continuous re-apply in `latex-modal.ts` (fix applied, NOT independently verifiable — needs live confirmation)**: `applyModalPosition` only ran once, ~60ms after the modal appeared; Excalidraw's modal is React-controlled and likely resets its own inline position style on every re-render (e.g. every keystroke). Added a `MutationObserver` watching the modal's own `style`/`class` attributes and re-applying position on change — mirroring the pattern the file already uses correctly for tooltip positioning. Guard logic compares the **live inline style values** against the target before writing (not a cached signature derived from the leaf rect, which was my first draft and wrong — a rect-based signature can't tell "already applied" from "got overwritten by React," since the rect can stay identical while the style gets stomped anyway). Also **not verifiable without `onload()` actually running** until this session's fix — same caveat as above.

4. **Warning banner + own live preview in `latex-modal.ts` (new feature, additive, lower risk)**: per the user's explicit choice (declined re-attempting the plugin-registry spoof that broke context-menu/shortcuts/blur-conversion in Parts 4→5). Found the real DOM structure by grepping the installed Excalidraw plugin bundle for the message key (`LATEX_SUITE_PLUGIN_SUGGESTION`) — it's rendered only when Excalidraw's own `this.latexsSuitePlugin` check fails, into a `.excalidraw-latex-suite-suggestion` div. `injectLivePreview()` now hides that div and renders our own live MathJax preview via Obsidian's own public `renderMath`/`finishRenderMath` API (same one `latex-suite/editor_extensions/math_tooltip.ts` already uses), updated on every keystroke in the modal's editor. Purely additive DOM work — never touches Excalidraw's plugin registry or internals, so it cannot regress context menu/shortcuts/blur-conversion the way the Part 4→5 spoof did.

5. **Incidental fixes surfaced by creating the missing `src/excalidraw/types.ts`** (see below): `graph-injector.ts` had a default-vs-named import bug (`import type ExpressionEngine from "../engine"` → `import type { ExpressionEngine }`) and a control-flow narrowing bug on a `Result<T>` check (`if (!x.ok || !x.value)` doesn't narrow `.error` in the branch — split into two `if`s). `snippet-engine.ts`'s `handleKeydown` field was typed `(e: KeyboardEvent) => void` instead of `(e: Event) => void` (inconsistent with the sibling `handleInput` field), breaking `addEventListener`'s overload resolution against the `HTMLTextAreaElement | HTMLInputElement` union parameter type. All three were real, previously-undetected type errors masked by the missing `types.ts` module (three files had `import type { SnippetDef, MathMode } from "./types"` pointing at a file that didn't exist — silently erased by esbuild since they're type-only imports, so the build never caught it, only `tsc` would have). Created `types.ts` with the exact shape inferred from actual usage across all three files.

### ⚠️ Needs live confirmation in Obsidian (in priority order)
1. Reload the plugin. **Open the console and confirm no errors during Excalidraw companion startup** — everything after the fixed crash line has been dormant for an unknown period; there could be a second landmine further into `onload()` that only surfaces now.
2. Type `mk` (math trigger) in an Excalidraw text element, then a display-math snippet, click away, and confirm it converts to a rendered SVG equation.
3. Right-click an existing LaTeX SVG element → "Edit LaTeX formula" (and double-click, and `Ctrl+\`) — confirm all three still open the modal correctly (these were the exact features the Part 4→5 spoof broke; this session's fix is purely additive and shouldn't touch them, but verify).
4. In that modal, confirm: the "Install Latex Suite" banner is gone, a live MathJax preview appears and updates as you type, and the modal stays positioned correctly (per your `latexModalPosition` setting) even after typing several characters.
5. Neither `obsidian-latex-suite` nor the old standalone `kings-excalidraw-math-companion` plugin is enabled in this vault (checked `community-plugins.json`) — no plugin-conflict concern there. `excalidraw-clutch-draw`/`excalidraw-extras` are enabled but are unrelated Excalidraw add-ons, not implicated.
