...
Optional 19MB WASM engine for limits, Taylor series, partial fractions, expand, and enhanced solving/integration.
...
### 2D Graphing
Explicit, implicit, parametric, polar curves, inequalities, and multi-equation overlays with interactive pan/zoom.
...
### 3D Graphing
WebGL surfaces, implicit surfaces (marching cubes), parametric curves, vectors, and points with click-to-interact rotation.
...
For advanced CAS operations (`@limit`, `@taylor`, `@partfrac`, `@expand`, enhanced `@solve`/`@int`):
