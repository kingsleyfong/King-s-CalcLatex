...
## Current Status: 🟢 v3.9.1 — perf fix: typing lag in large .md notes root-caused to vendored LaTeX Suite's unbounded per-keystroke document slice; shipped, needs live confirmation

### What Happened (Part 54 — perf audit: typing lag in large .md notes with many equations, root-caused to vendored LaTeX Suite's O(cursor-position) per-keystroke slice)
User (mid-way through undergrad finals, explicitly time-pressured) asked to audit and fix reported lag while typing inside `$...$` in "very heavy/long" markdown notes with lots of equations, with an explicit "won't break current functionality" requirement given exam stakes. Framed as a full audit, not a quick guess-and-patch.

Methodically checked every layer active while typing in math mode, in order, verifying each was actually fine before moving on (rather than assuming a fix without evidence):
1. `editor/decorations.ts`'s StateField `update()` -- confirmed correctly scoped to only `changedLines` (matches antipattern #1 already documented), not a full-document rebuild.
2. `editor/widgets.ts`'s `ResultWidget` -- confirmed `eq()` is implemented correctly (compares latex+mode content, not reference identity) and `toDOM()` defers evaluation via `Promise.resolve().then(...)`, never blocking synchronously.
3. `latex-suite/utils/context.ts`'s `updateMathBounds()` -- confirmed it iterates the syntax tree scoped to `view.visibleRanges` only (viewport-limited, not whole-document), and `computeEquationBounds()` uses bounded tree-cursor sibling traversal (`.prev()`/`.next()`), not a text scan.
4. `latex-suite/features/run_snippets.ts` -- **found the actual bug**: `getSliceAroundCursor()` calls `view.state.sliceDoc(0, to)` unconditionally on every keystroke, materializing the ENTIRE document from position 0 up to the cursor as a JS string, every single time the snippet-matching engine runs (which fires on every keystroke while editing). For a document with substantial content before the cursor, this is a real, repeated O(cursor position) cost.

Before touching this (vendored code, per this project's own CLAUDE.md antipattern 0(e) -- diff against upstream first), diffed against the real `artisticat1/obsidian-latex-suite` repo: **byte-identical**, confirming this isn't a fork regression, it's upstream's own long-standing behavior. Searched upstream's issue tracker for external corroboration and found **issue #320** (still open): "LaTeX Suite will significantly affect Obsidian's performance" on documents over ~10-25k characters, with the reporter's own profiling pointing at the same general area -- an exact match for the user's symptom, confirming this wasn't a guess. Also found merged PR #450 ("Feat/optimize snippets") which optimized a *different* part of the pipeline (`view.dispatch` batching during expansion) -- confirmed via reading the diff that it does NOT touch `sliceDoc(0, to)`, so this specific bug remains genuinely unfixed upstream as of the latest commit checked (2026-06-13).

**The fix required care beyond just bounding the slice**, since `runSnippetCursor()`'s downstream position math (`triggerPos`, `triggerEndPos`, fed into `isOnWordBoundary()` and `queueSnippet()`) implicitly assumes the slice starts at document position 0 -- shrinking the window without correcting for this would silently compute wrong absolute positions, corrupting snippet expansion (a far worse regression than the lag). Worked through the exact position-math contract for all three snippet types (`VisualSnippet`/`RegexSnippet`/`StringSnippet`):
- `VisualSnippet.process()`'s `triggerPos` comes from `range.from` -- **already an absolute document position**, independent of the slice, must NOT be offset-adjusted.
- `RegexSnippet`/`StringSnippet`'s `triggerPos`/`triggerEndPos` are computed relative to the slice's own start (0-based within `effectiveLine`) -- **must** have `windowStart` added back to recover the true absolute position.
- Verified this rule against a concrete worked example (hypothetical 2010-character document, cursor at 2010, 2-character trigger) before trusting it: closed-form derivation and the worked example agreed exactly.

**Implementation**: introduced `SNIPPET_CONTEXT_WINDOW = 2000` (generous margin -- built-in/realistic custom triggers are at most a few dozen characters), bounded both the backward slice (`sliceDoc(windowStart, to)` where `windowStart = max(0, to - 2000)`) and the previously-unbounded forward slice (`effectiveLineAfter`, used for `^`-anchored `triggerAfter` matching, was `sliceDoc(to)` all the way to document end -- now also windowed). `getSliceAroundCursor()` now returns `windowStart` alongside `line`/`effectiveLineAfter`; `runSnippetCursor()` adds it back to `triggerPos` (except for `VisualSnippet`, per the rule above) and to `triggerEndPos` when defined. **Key safety property**: for any document/cursor position where fewer than 2000 characters precede the cursor (i.e. the overwhelming common case, and *all* short/normal-sized documents), `windowStart` computes to exactly `0`, making the change a complete no-op -- behavior is byte-for-byte identical to the original code in that case. The bug (and the fix's effect) only exists for documents where 2000+ characters precede the cursor.

Also found a second, unrelated unbounded-slice pattern while sweeping the rest of `latex-suite/` for the same issue: `context.ts`'s `getEquations()` does `state.sliceDoc(0, state.doc.length)` (the *entire* document) -- but this is a **KCL fork addition** (not upstream-inherited), gated behind `alwaysMathFacet` (see antipattern #22), which only activates inside the Excalidraw "Edit LaTeX" modal's foreign CM6 editor, not the main markdown editor -- and it's memoized (`if (this.equations) return this.equations`), not re-run every keystroke. Given the user's reported symptom is specifically about long `.md` notes (not the Excalidraw modal), this doesn't match the complaint and carries much lower real-world severity (single-equation buffers in that modal are typically far smaller than a whole note) -- **deliberately left untouched**, flagged as a minor deferred item rather than fixed opportunistically.

Verified via the same `tsc --noEmit` full-baseline comparison used throughout this session: zero new errors (exact same pre-existing baseline). Build clean. Shipped as v3.9.1 (patch -- bug/perf fix, not a new feature).

**Needs live confirmation from user** (cannot be verified in this environment -- no way to run Obsidian interactively here): test snippet expansion (a string trigger, a regex trigger, and a visual/selection-based snippet if one is in regular use) in both a short document and a genuinely large one (paste/generate several thousand characters before the test equation) to confirm expansion still lands in the correct position and nothing broke; separately confirm the reported lag is actually gone when typing near the end of a large real note.

### What Happened (Part 53 — new feature: Excalidraw drawing snapshot/restore commands, born from the equation-resize investigation)
Directly downstream of the Part 52 hotfix conversation: user reported (separately from the blank-settings bug) that LaTeX SVGs they'd manually resized on Excalidraw canvases were reverting to default size after reloading Obsidian, specifically threatening exam-critical formula sheets. Diagnosed via reading Excalidraw's own bundled `main.js` (not King's CalcLaTeX code): Excalidraw doesn't store rendered equation images in the `.excalidraw.md` file, only the LaTeX source -- on every file load it regenerates the image fresh and is *supposed* to rescale it back to the element's current on-canvas size via a `shouldScale: true` flag, but that rescale step is fragile and some equations silently come back at natural (unscaled) size. Confirmed via user's own live isolation test (manual real-OK-click saves, both no-edit and with-edit, both preserved size correctly) that this is NOT King's CalcLaTeX's edit/save-modal code -- ruled out by testing, not just by reading code. Confirmed via full codebase grep that King's CalcLaTeX has exactly one `ea.addLaTex()` call site (new-equation creation only) and zero code touching equation width/height/scale anywhere, including nothing on file-load/app-reload. This is upstream Excalidraw plugin territory, not something patchable from our side.

As an immediate stopgap (session before this one), built external Node.js scripts (`extract.js`/`restore.js`, using the `lz-string` npm package matching Excalidraw's own compression) that decompress/recompress the `.excalidraw.md` file's internal format directly, letting a corrupted file's sizes be surgically restored from a pre-corruption snapshot without any Obsidian involvement. Verified byte-perfect round-trip fidelity (0 mismatches across 234+478+48 elements from the user's three real formula sheets) before trusting the approach. Backed up the user's actual formula sheet files to `_backups/excalidraw-formula-sheets-2026-07-31/` in the vault (not in this repo) as an immediate safety net.

User then asked whether this could become a proper King's CalcLaTeX feature: a Command-Palette-triggered snapshot/restore, saved to the same folder as the drawing with a timestamped filename. Recommended (and built) a **better architecture than the external-script version**: instead of parsing Excalidraw's internal compressed-file format (an undocumented implementation detail that could change between Excalidraw versions without notice), use `ExcalidrawAutomate`'s own public, stable `getSceneElements()`/`updateScene()` API -- the same pattern already used throughout this codebase (`companion-manager.ts`, `graph-injector.ts`). This works on the *live in-memory scene* (capturing unsaved in-session state too, not just what's on disk) and needs zero new dependencies.

**Implementation** (`src/excalidraw/snapshot-manager.ts`, new file):
- `snapshotCurrentDrawing()`: gets the active Excalidraw view via the same `activeLeaf.view?.getViewType() === "excalidraw"` pattern used elsewhere, reads `api.getSceneElements()`, captures every non-deleted element's `type`/`x`/`y`/`width`/`height`/`angle`/`scale` (user chose "all elements," not just equations, for broader protection at negligible cost), writes a `<name>.snapshot.<timestamp>.json` file to the same folder as the drawing via `vault.create()` (public API, matches this session's earlier fs→vault.adapter compliance work).
- `restoreFromSnapshot()`: finds all `<name>.snapshot.*.json` files in the drawing's folder, shows a `FuzzySuggestModal<TFile>` picker (user chose picker-over-auto-newest, since the newest snapshot could itself have been taken post-corruption), then surgically overwrites only the elements whose position/size differs from the chosen snapshot -- anything added/removed since is left untouched -- and calls `api.updateScene({elements: updated})` to apply (matches `graph-injector.ts`'s existing mutate-and-updateScene pattern, no `addElementsToView` needed since these are existing-element mutations, not new-element creation).
- Two commands registered in `main.ts` next to the existing ones, following the exact same `addCommand({id, name, callback})` shape.
- One implementation snag: `import { moment } from "obsidian"` type-checked as "not callable" (a namespace-import typing quirk with this project's `@types/moment` version) -- sidestepped by writing a small local `formatTimestamp()` using plain `Date` methods instead of fighting the type system for something this simple.

Verified via the same `git stash`/full-`tsc` baseline-comparison technique used throughout this session: zero new typecheck errors, build clean. Shipped as v3.9.0 (**minor**, not patch -- this is a genuine new feature, matching this project's semver convention).

**Needs live confirmation from user**: run the snapshot command on a real drawing, confirm the JSON file appears in the right folder with sensible timestamp/content; then (ideally after a reload that reproduces the original bug) run restore and confirm the picker shows the snapshot and sizes come back correctly.
